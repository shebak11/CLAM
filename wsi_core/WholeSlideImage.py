            ...
            if len(asset_dict) > 0:
                if init:
                    ...
                    for idx in range(asset_dict['coords'].shape[0]):
                        element= {'coords' : asset_dict['coords'][idx].tolist() ,
                                         'path' : "WSI/TCGA/COADtest_dir4/stitchPatches" +'/' +self.name + '/' +self.name + "+idx" '+'+str(asset_dict['coords'][idx][0]) + '+'+ str(asset_dict['coords'][idx][1]) + '.jpg'}
